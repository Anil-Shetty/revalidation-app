...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const results = await Promise.all(
        sdk.parameters.installation.pageTemplates.map((template: ContentTypeProps) =>
          sdk.cma.entry.getMany({ query: { content_type: template.sys.id } }),
        ),
      )
  ...
  const filterEntries = (type: string, query: string) => {
    startTransition(() => {
      const filteredEntriesByType =
        type === 'all' ? entries : entries.filter((entry) => entry.sys.contentType.sys.id === type)
      const filteredEntriesByQuery = filteredEntriesByType.filter((entry) => {
        ...
        const contentType = sdk.parameters.installation.pageTemplates.find(
          (ct: ContentTypeProps) => ct.sys.id === entry.sys.contentType.sys.id,
        ) as ContentTypeProps
        if (
          entry.fields[contentType.displayField] &&
          entry.fields[contentType.displayField][sdk.locales.default]?.includes(query)
        ) {
          return true
        }
        return false
    ...
    <div className="my-20 mx-auto max-w-5xl">
      ...
      <div className="my-4">
        <CollectionSearchFilter
          contentTypes={sdk.parameters.installation.pageTemplates}
          entries={entries}
          onContentTypeChange={onContentTypeChange}
          onSearchQueryChange={debouncedOnSearchQueryChange}
        />
      </div>
      <CollectionTable
        contentTypes={sdk.parameters.installation.pageTemplates}
