...
const CollectionSearchFilter = ({ contentTypes, entries, onContentTypeChange, onSearchQueryChange }: CollectionSearchFilter) => {
  ...
  return (
    <Form className='flex p-1 border border-gray-100 rounded-lg focus-within:border-primary focus-within:shadow-focus'>
      <FormControl className='flex gap-4 items-center m-0 pl-3 bg-gray-400 hover:bg-gray-200 rounded-lg'>
        ...
        <Select id="contentType" name="contentType" className='content-type-filter' defaultValue='all' onChange={onContentTypeChange}>
          {selectOptions.map(item => (
            <Select.Option key={item.name} value={item.value} className='bg-white text-blue-900'>{item.name}</Select.Option>
          ))}
        </Select>
      </FormControl>
